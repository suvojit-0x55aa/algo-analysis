...
class CPM
{
    ...
    CPM(int n)
    {
        ...
        source = 2 * n;
        sink   = 2 * n + 1;
        G = new WeightedDigraph(2 * n + 2);
    }

    public void add_job(int id, double duration)
    {
        G.add_edge(new WeightedEdge(source, id,     0.0));
        G.add_edge(new WeightedEdge(id,     id + n, duration));
        G.add_edge(new WeightedEdge(id + n, sink,   0.0));
    }

    public void add_precedence(int id, int precendent)
    {
        G.add_edge(new WeightedEdge(id + n, precendent, 0.0));
    }

    public void calc_cpm()  { lp = new AcyclicLP(G, source); }

    public double job_start(int id)  { return lp.dist_to(id);     }
    public double job_end  (int id)  { return lp.dist_to(id + n); }
    public double tot_time ()        { return lp.dist_to(sink);   }
}
...
class RunApp
{
    public static void main(String arg[])
    {
        ...
        CPM jobSch = new CPM(n);
        for (int i = 0; i < n; ++i)
        {
            double duration = readIn.nextDouble();
            jobSch.add_job(i, duration);

            int m = readIn.nextInt();
            for (int j = 0; j < m; ++j)
            {
                int pre = readIn.nextInt();
                jobSch.add_precedence(i, pre);
            }
        }
        ...
        //Print Schedule
        for (int i = 0; i < n; ++i)
            System.out.println("Job : " + i + " Start : " + jobSch.job_start(i) + " End : " + jobSch.job_end(i));
        System.out.println("Total Time : " + jobSch.tot_time());
    }
}